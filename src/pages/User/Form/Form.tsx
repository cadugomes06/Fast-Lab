...
const Form = () => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      setFileUrl(prevfileUrl => [...prevfileUrl, ...files])
    }
  };
  ...
  useEffect(() => {
    ...
    const uploadPromises = fileUrl.map(async (file: any) => {
          ...
          <div className="w-full h-[100vh] bg-black/50 flex justify-center items-center fixed z-20">
            <div className="w-[38rem] h-[18rem] relative bg-white rounded-md flex flex-col md:w-[32rem] sm:w-[20rem]">
              ...
              <div className="w-full max-h-max py-8 px-6 sm:px-4 sm:py-4">
                <p className="font-regular text-md text-teal-600 sm:text-sm">No momento seu convênio não está disponível para a modalidade de pré cadastramento. Será necessário comparecer a uma unidade Hemolabes para autorizar seus exames por meio de carteirinha ou token gerado no aplicativo Unimed cliente.
